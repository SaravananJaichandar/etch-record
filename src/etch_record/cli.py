...
from .custody_export_client import (
    CustodyExportError,
...
# ---------------------------------------------------------------------------
# Wave 5 #19 (2026-08-02) - chain-of-custody export bundle
# ---------------------------------------------------------------------------
@click.option(
    "--custody-export-session", "custody_export_session",
    type=str, default=None,
    help=(
        "Wave 5 #19: OSS session_id to bundle for FRE 902 / eIDAS "
        "self-authenticating export. Triggers POST "
        "/v1/etch-chain/custody-export. Requires "
        "--custody-export-declaration-regime."
    ),
)
@click.option(
    "--custody-export-declaration-regime",
    "custody_export_declaration_regime",
    type=click.Choice((
        "fre_902_11", "fre_902_13", "fre_902_14",
        "eidas_qualified",
    )),
    default=None,
    help=(
        "Wave 5 #19: declaration regime for the export bundle. "
        "fre_902_11 = certified records of a regularly conducted "
        "activity. fre_902_13 = records generated by an electronic "
        "process or system. fre_902_14 = data copied from an "
        "electronic device. eidas_qualified = EU 910/2014 qualified."
    ),
)
@click.option(
    "--custody-export-out", "custody_export_out",
    type=click.Path(path_type=Path), default=None,
    help=(
        "Wave 5 #19: optional path to write the full bundle JSON. "
        "When omitted, only the marker seq + bundle hash are echoed "
        "to stdout (the bundle stays server-side, retrievable via "
        "the marker)."
    ),
)
...
def main(
    ...
    # Wave 5 #19 - chain-of-custody export bundle. Independent.
    if _custody_export_wanted(
        custody_export_session,
        custody_export_declaration_regime,
    ):
        _validate_custody_export_flags(
            custody_export_session,
            custody_export_declaration_regime,
        )
        try:
            r = record_custody_export(
                cfg=cfg, oss_event_id=event_id,
                session_id=custody_export_session,
                declaration_regime=(
                    custody_export_declaration_regime
                ),
            )
        except CustodyExportError as exc:
            click.echo(
                f"etch-record: custody-export: {exc}", err=True,
            )
            sys.exit(17)
        click.echo(
            f"OK  custody_export_marker_seq="
            f"{r.custody_export_marker_seq}  "
            f"bundle_hash={r.manifest_bundle_hash}  "
            f"declaration_regime="
            f"{custody_export_declaration_regime}",
        )
        if custody_export_out is not None:
            import json as _json
            custody_export_out.write_text(_json.dumps(
                {
                    "custody_export_marker_seq":
                        r.custody_export_marker_seq,
                    "custody_export_marker_row_id":
                        r.custody_export_marker_row_id,
                    "manifest_bundle_hash":
                        r.manifest_bundle_hash,
                    "oss_event_id_ref": r.oss_event_id_ref,
                    "bundle": r.bundle,
                },
...
def _custody_export_wanted(
    session_id: str | None,
    declaration_regime: str | None,
) -> bool:
    return any((session_id, declaration_regime))


def _validate_custody_export_flags(
    session_id: str | None,
    declaration_regime: str | None,
) -> None:
    missing = []
    if session_id is None:
        missing.append("--custody-export-session")
    if declaration_regime is None:
        missing.append("--custody-export-declaration-regime")
    if missing:
        raise click.UsageError(
            "Wave 5 #19 custody-export requires: "
            + ", ".join(missing),
        )
